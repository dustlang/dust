...
Constraints are central to the Φ-regime but are also permitted in K-regime where they govern effect legality and execution eligibility.
...
K-regime constraints are often used to:
	•	validate inputs,
	•	enforce safety bounds,
	•	restrict effect legality.
...
In Q-regime:
	...
	•	Constraints MUST NOT introduce copying or observation.
	•	Constraint predicates MUST be evaluable without violating linearity.

Constraint failure in Q-regime invalidates the process and MUST be reported statically where possible.
